...
func (s *ShortURL) ReadURL(req *http.Request) ([]byte, error) {
	...
	if err != nil {
		s.Logger.RaiseError(err, "ShortURL.ReadURL>DB.Read", nil)
		return EmptyByteSlice, ErrDataNotValid
	}
	...
	default:
		s.Logger.RaiseError(err, "ShortURL.ReadURL>DB.Read>switch", nil)
		return EmptyByteSlice, ErrDataNotValid
	}
}
...
func (s *ShortURL) CheckDB(req *http.Request) ([]byte, error) {
	...
	if err != nil {
		s.Logger.RaiseFatal(err, "ShortURL.CheckDB>Ping", nil)
		return EmptyByteSlice, err
	}
	return StoreDBIsSucces, nil
}
